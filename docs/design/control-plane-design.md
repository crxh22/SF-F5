...
**Status:** design, v1.11 — 2026-06-12, revised after adversarial review + contract amendments CCR-1, CCR-2, CCR-3, CCR-5, CCR-6, CCR-7, CCR-8, CCR-9, CCR-10, CCR-11 (see Review log). Binding spec: `_FRAMEWORK_MVP_DoD.md` (DoD v3); governed by `00 - DOCTRINA.md`; parameters: `factory.config.yaml`; constraints from decision log D-0002/D-0003/D-0007/D-0009.
...
def mark_process_running(conn, process_id: int, *, pid: int, at: str) -> None   # 'spawned'→'running': persist pid post-exec, heartbeat_at=at as initial heartbeat; FactoryError on rowcount != 1 (CCR-2)
MIGRATIONS_DIR: Path   # packaged migrations directory (ratified post-wave-1; imported by frozen conftest.py and cli.init)
...
1. **Spawn**: `run_agent` enforces kind/cp_id/role tagging (§2 creep-scan precondition), resolves `config.models[role]` + the role-class canon bundle (D-0009) → adapter builds argv (`claude --model <m> --output-format stream-json --verbose --append-system-prompt <canon> [--resume <session_id>] -p <prompt>`; codex equivalent per OPEN-3 with AGENTS.md materialized into cwd; `stub` for tests). The child starts in its **own process group** (`start_new_session=True`; Linux backstop `PR_SET_PDEATHSIG=SIGKILL` via preexec_fn) with stderr redirected at spawn to `<ndjson_log_dir>/proc-<12hex>.stderr` (log paths are token-named and recorded in the registry row at insert — the column is authoritative, CCR-2 note; inherited fd — no drain task, crash-safe evidence; stderr is never PIPEd without a drain — a full 64KB pipe deadlocks the child into a spurious timeout — and never merged into stdout, which would corrupt the NDJSON stream). Insert `process_registry` row (`spawned`) and open `<ndjson_log_dir>/proc-<12hex>.ndjson` **before** exec; flip to `running` + persist pid + initial heartbeat via `db.mark_process_running` after exec (CCR-2). No subprocess without timeout + terminate/kill (harvested invariant).
...
**CCR-11 (contract change request #11), 2026-06-12 — founder-approved option A (D-0037), v1.10→v1.11.** Capacity governor: on a usage-limit signature match (CCR-6 detector, which now returns the matched signature) the scheduler enters a HOLD — no new claude-route spawns (codex/stub/Tier-1 proceed; ESCALATED pickup always runs), `capacity_hold_started` event + the existing founder page; a `models.capacity_probe` canary (haiku, print, tools:none) runs every `capacity_governor.probe_interval_s` (bounded ≤ staleness/2, never escalates, ordinary registry+ledger run); on probe success the hold lifts atomically (`capacity_hold_ended` + RO page „Capacitate revenită — fabrica a reluat singură") and the governor AUTO-RESOLVES — actor `capacity_governor`, via the standard resolution machinery — exactly the open stage escalations with trigger `agent_run_failed` AND evidence `usage_limit: true` (the incident-7 gate now limit-marks its evidence), token mapped from the pre-escalation step (SPEC/BUILD/VALIDATE→rework:*, AUDIT→rework:VALIDATE — no rework:AUDIT in the vocabulary, re-validation cost accepted; MERGE_GATE deliberately unmapped → architect). Anything else — other triggers, unmarked failures, phase rows — NEVER auto-resolved (test-pinned trust boundary). `capacity_governor.enabled: false` is byte-identical to pre-CCR-11 behavior (pinned); golden config enables it. Restart reconcile closes orphaned hold pairs; an outage surviving a restart re-enters hold on the first dead spawn (documented gap). Verifier (approve_with_notes) residuals registered: forever-failing probe = indefinite silent drain visible only on the dashboard (re-page candidate, trigger = first such episode); `--until-blocked` runs exit during a hold without resuming; probe spend lands in global totals but on no per-unit /costuri table; probe timeout bound and tiny-interval guard not test-pinned; VALIDATE holds even under a hypothetical codex validator (cp1 role is claude — conservative by design); phase spawns remain ungated by incident-7 (D-0036 watch item — a limit-killed phase architect holds the factory but needs manual triage).

**CCR-10 (contract change request #10), 2026-06-12 — approved with dashboard-design v1.3 (D-0036), v1.9→v1.10.** Additive: `config.ModelPrice`/`PricingCfg` + optional top-level `pricing` (founder-tunable USD/MTok by LEDGER model string — Doctrine §14); `db.list_token_ledger` + `db.sum_token_cost` (pure-read repository helpers, ledger-id order); dashboard read-only refresh-free route `GET /costuri` (per-agent cost tables; main page keeps stateless summaries — the v1.2 A-1 refresh-collapse class generalized into a standing constraint); §2b factory total and every cost figure render the exact/`~`-estimated PAIR, never merged (codex routes report no cost — config prices estimate them). Also under this deploy window: **incident-7 gate** (D-0035 root cause): `_run_step_agent` refuses to let any step consume artifacts of a failed run (exit≠0/killed/timeout without a declared-failure sentinel) — new scheduler-literal escalation trigger `agent_run_failed` → phase_architect with evidence + worktree discard of the dead run's uncommitted writes (committed work untouched — §5.5d preserved); contained by a private exception caught only in `StageExecutor.execute` (siblings unaffected, §6 boundary untouched). Verifier-confirmed watch items: phase-level spawns (`_step_planning`, phase tier2) bypass the gate — same class, own slice on first incident; dual-AUDIT sibling abandonment leaves a fail-visible (never silent) §3.1 wedge possibility.

**CCR-9 (contract change request #9), 2026-06-12 — approved post-incident (D-0033), v1.8→v1.9.** Agent-facing re-entry context becomes an EXPLICIT transition-payload key `rework_context`, never the generic merged `reason` (statemachine merges the transition reason into every stored payload, so reason-driven prompt captions leaked routine strings: every fresh BUILD prompt rendered "Rework context: spec artifact registered." — latent since wave 3; caught when the first verifier round REJECTED the initial reason-key implementation of this CCR for the same defect class on fresh SPEC dispatch). Set at exactly the genuine stage-level rework re-entries: escalation resolution (operator `--reason` rationale via new `_resolution_reason` event read, or `escalation resolved: <resolution>` fallback — the operator rationale finally REACHES the re-entered role, the D-0033/escalation-6 incident need), audit-comply→BUILD, CP-1 rebuild/respec, CP-1 continue_session→rebuild downgrade. Consumed only by `_build_prompt` and the §-amended `_spec_prompt`, which now mirrors the BUILD pattern: 4th param entry payload, "Rework context:" caption + "Read first:" extras (validation-report.md, escalation-payload.md, build-notes.md when present). Fresh dispatch and spec-registered→BUILD prompts pinned context-free; validator stays clean-context (§3.1, no entry payload). Registered consequences: Tier-1/Tier-2 routed-back BUILD re-entries keep their Read-first artifacts but no caption (additions ride future incidents); Tier-1 tests-failed is the weakest such path (no path-specific artifact) — watch item; CP-1 rationale (consultations table) still unplumbed to prompts — trigger: first respec/rebuild where its absence bites. Two-round gauntlet: reject (fresh-entry regression + synthetic-fixture pin) → rework → approve_with_notes (766 tests, ruff clean). §5.1 argv-literal semantic amendment: agent prompts travel via **stdin**, never argv (claude argv ends bare `-p`; codex `exec`/`exec resume` end `-`; stub symmetric; `CliAdapter.build_cmd` signature unchanged) — a single argv element is kernel-capped at MAX_ARG_STRLEN ≈128KB, and the §3.1 Tier-2 input (bounded 300KB/unit) plus CP-1 (200KB) both exceed it: `[Errno 7] Argument list too long` killed the first real Tier-2 spawn (D-0028 incident 1). `run_agent` spawns stdin=PIPE with a concurrent bounded feeder (write+drain+close; BrokenPipe contained as the early-exit path; OPEN-3's "codex reads piped stdin" is now the deliberate channel). Also under this CCR: the §3.1 BUILD step accepts a no-changes/no-sentinel agent exit as `build_noop_accepted` → VALIDATE (validation is the arbiter; the committed-changes contract broke §5.5d idempotent re-entry — D-0028 incident 2); builder prompts forbid agent self-commits. CLI-verified invocations + a 204KB live-stdin probe recorded in the verification log.
...
**CCR-2 (contract change request #2), 2026-06-10 — approved, v1.2→v1.3.** Wave-2 runner builder: no repository function could flip `process_registry` 'spawned'→'running' or persist the child pid post-exec (insert is pre-exec, finalize is terminal-only — misusing it would corrupt `last_session_id`'s finalized predicate), so the §5.5a cross-restart orphan sweep had no pids to find. Added `db.mark_process_running(conn, process_id, *, pid, at)`; architect amendment: `at` writes `heartbeat_at` as the initial heartbeat so staleness math is sound from exec time. Also ratified: token-named log files (`proc-<12hex>.*` — the registry column is authoritative), `MIGRATIONS_DIR` as public db surface (wave-1 verifier finding), ro-URI quoting fix folded into the delta. Decision log D-0013.
