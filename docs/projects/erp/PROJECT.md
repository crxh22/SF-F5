...
- **Stage granularity is law (D-0050, founder directive 16-06-2026 — binding for ALL domain phases):** decompose into FINER stages than foundation's. Foundation shipped several oversized stages (auth-access ~237M, status-notifications ~231M, register-schemas ~154M tokens) that drove 250M-cap pressure AND a 1M-context integration_validator overflow (large sibling-diff payloads). **Target: no stage projected >~100–120M tokens** — well under the 250M cap, and keeps merge/phase-gate sibling diffs small. Prefer splitting a subsystem along its natural seams (per entity / per producer / per flow) over one broad stage; where this conflicts with "stages sized at the upper bound of one-pass builder confidence", **take the smaller**. Build agents are opus (capable), so fine decomposition is well-supported. (Factory log D-0050; the foundation evidence is the as-built token record.)
