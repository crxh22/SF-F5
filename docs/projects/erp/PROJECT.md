...
- Workspace skeleton: Django+PostgreSQL backend, React+TS frontend (monorepo `backend/`+`frontend/`), test harness (`scripts/test.sh` becomes real: `uv run pytest`), CI-grade lint/type config.
